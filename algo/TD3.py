...
class TD3:
    def __init__(self, state_size, action_size, action_upper_bound, action_lower_bound):
        ...
        self.action_upper_bound = torch.tensor(action_upper_bound).to(self.device)  # action space upper bound
        self.action_lower_bound = torch.tensor(action_lower_bound).to(self.device)  # action space lower bound
        self.create_actor()
        self.create_critic()
    ...
    def update(self, buffer, n_iter, batch_size):
        ...
        Args:
            ...
            n_iter: number of critic networks update
            batch_size: size of sampled batch from replay buffer
        """
        
        # Perform n_iter times of updates of the critic networks and 
        # (n_iter / policy_delay) times of updates of the actor network
        for it in range(n_iter): 
            ...
            done_batch: torch.Tensor with shape (self.batch_size, 1), a mini-batch of 0-1 integers, 
            ...
            reward_batch = torch.FloatTensor(reward).reshape((batch_size,1)).to(self.device)
            next_state_batch = torch.FloatTensor(next_state).to(self.device)
            done_batch = torch.FloatTensor(done).reshape((batch_size,1)).to(self.device)
            ...
            y = reward_batch + (1 - done_batch) * self.gamma * target_Q
