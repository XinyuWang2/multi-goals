...
"""
Simple Replay Buffer (using deque, store transitions)
"""
class SimpleBuffer():
    def __init__(self, size):
        self.buffer = deque(maxlen=size)  # replay buffer capacity
    def add(self, state, action, reward, done, next_state, info):
        self.buffer.append((state, action, reward, next_state, done))
    def sample(self, batch_size):
        random_sample = random.sample(self.replay_memory_buffer, batch_size)
        return random_sample
    
    @property
    def size(self): # size for batched sample 
        return len(self.buffer)

"""
Hindsight Replay Buffer (store episode transitions)
TODO: Now it is not adding next state in its buffer 
"""
